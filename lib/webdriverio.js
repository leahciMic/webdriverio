...
module.exports = function WebdriverIO(args, modifier) {
    ...
    function unit(lastPromise) {
        ...
        client.addCommand = function(fnName, fn, forceOverwrite, disableCallback) {
            if(client[fnName] && !forceOverwrite) {
                throw new ErrorHandler.RuntimeError('Command "' + fnName + '" is already defined!');
            }
            return unit.lift.call(null, fnName, fn, disableCallback);
        };
     ...
     */
    unit.lift = function (name, func, disableCallback) {
        prototype[name] = function () {
             ...
             */
            if(!disableCallback && !name.match(/(selector)*(E|e)xecute(Async)*/) && !name.match(/waitUntil/) && typeof args[args.length - 1] === 'function') {
                callback = args.pop();
